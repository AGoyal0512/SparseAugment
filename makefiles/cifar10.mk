## cifar10.Standard.ERK.%: Run CIFAR 100 ERK runs. % in RigL, SNFS, SET
cifar10.Standard.ERK.%:
	${PYTHON} main.py hydra/launcher=basic dataset=CIFAR10 optimizer=SGD masking=$* \
	+specific=cifar100_resnet50_masking seed=$(SEED) exp_name="$*_Standard" masking.density=$(DENSITY) \
	wandb.entity="agoyal25" wandb.project="SparseAugment" wandb.use=$(USE_WANDB) \
	dataset.augment="standard" $(HYDRA_FLAGS)

## cifar10.Base.ERK.%: Run CIFAR 100 ERK runs. % in RigL, SNFS, SET
cifar10.Base.ERK.%:
	${PYTHON} main.py hydra/launcher=basic dataset=CIFAR10 optimizer=SGD masking=$* \
...
## cifar10.CutMix.ERK.%: Run CIFAR 100 ERK runs. % in RigL, SNFS, SET
cifar10.CutMix.ERK.%:
	${PYTHON} main.py hydra/launcher=basic dataset=CIFAR10 optimizer=SGD masking=$* \
...
## cifar10.MixUp.ERK.%: Run CIFAR 100 ERK runs. % in RigL, SNFS, SET
cifar10.MixUp.ERK.%:
	${PYTHON} main.py hydra/launcher=basic dataset=CIFAR10 optimizer=SGD masking=$* \
