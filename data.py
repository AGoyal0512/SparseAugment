...
def _get_CIFAR10_dataset(root: "Path", augment: str = "standard") -> "Tuple[Dataset,Dataset]":
    """
...
def _get_CIFAR100_dataset(root: "Path", augment: str = "standard") -> "Tuple[Dataset,Dataset]":
    """
    ...
    print(f"Inside CIFAR100 Dataloader: {augment}")

    if augment == "base":
        train_transform = transforms.Compose([transforms.ToTensor(), normalize])

    elif augment in ["cutmix", "mixup", "standard"]:
        train_transform = transforms.Compose(
            [
...
def _get_Mini_Imagenet_dataset(root: "Path", augment: str = "standard") -> "Tuple[Dataset,Dataset]":
    """
...
def _get_MNIST_dataset(root: "Path", augment: str = "standard") -> "Tuple[Dataset,Dataset]":
    """
...
def get_dataloaders(
    ...
    print(f"Inside get_dataloaders: {augment}")
    full_dataset, test_dataset = registry[name](Path(root), augment)
